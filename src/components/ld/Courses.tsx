...
export function Courses({ onNav, initialQuery }: { onNav: Nav; initialQuery?: string }) {
  ...
  const query = norm(q);
  // Strict 3-state model — only Not started / In progress / Completed. The
  // queries layer already caps progress at 99 when the agreement is unsigned,
  // so "in progress" naturally covers the "video + quiz done but unsigned"
  // case without a separate amber state.
