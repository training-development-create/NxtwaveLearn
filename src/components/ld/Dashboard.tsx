  ...
  return (
    <div onClick={onClick} style={{
      ...
      <div style={{flex:1, minWidth:0}}>
        <div style={{fontSize:14, fontWeight:700, color:'#0A1F3D'}}>{c.title}</div>
        {(() => {
          // Single-line meta: videos · duration · pub date · due date.
          // Dates rendered as compact "12 Apr" — no labels, due coloured only
          // when overdue/soon so the row stays clean.
          ...
          const pub = shortDate(c.published_at);
          const due = shortDate(c.due_at);
          return (
            <div style={{fontSize:12, color:'#5B6A7D', marginTop:2, display:'flex', gap:8, flexWrap:'wrap', alignItems:'center'}}>
              <span>{c.lessons_total} videos · {c.duration_label || '—'}</span>
              {pub && <span>· 📅 {pub}</span>}
              {due && <span style={{color:dueColor, fontWeight: overdue || dueSoon ? 700 : 400}}>· ⏰ {due}</span>}
            </div>
          );
        })()}
      </div>
